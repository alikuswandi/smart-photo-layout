...
function getPaperSize(s=settings()){if(s.paperSize==='CUSTOM')return {w:s.customPaperW,h:s.customPaperH,name:`Custom ${s.customPaperW}×${s.customPaperH} mm`};return PAPER_SIZES[s.paperSize]||PAPER_SIZES.A4}
...
function addCustomToQueue(photoId,w,h){const source=state.photos.find(x=>x.id===photoId);w=Number(w);h=Number(h);if(!source)return;if(!Number.isFinite(w)||!Number.isFinite(h)||w<5||h<5||w>1000||h>1000)return alert('Ukuran foto custom harus antara 5–1000 mm.');const queuePhotoId=gid(),sourceRootId=source.sourcePhotoId||source.id;state.photos.push({...source,id:queuePhotoId,sourcePhotoId:sourceRootId,isQueueClone:true});state.adjustments[queuePhotoId]={...DEFAULT_ADJ,...(state.adjustments[photoId]||{})};state.queue.push({id:gid(),photoId:queuePhotoId,sourcePhotoId:sourceRootId,presetId:'custom',w,h,name:`Custom ${w} × ${h} mm`,qty:1});renderQueue();invalidate()}
function renderPhotos(){const el=$('#photoLibrary'),libraryPhotos=state.photos.filter(p=>!p.isQueueClone);if(!libraryPhotos.length){el.innerHTML='<div class="empty-mini">Belum ada foto.</div>';return}el.innerHTML=libraryPhotos.map(p=>`<div class="photo-card"><div class="photo-head"><img class="thumb" src="${p.aiCutoutDataUrl||p.dataUrl}"><div class="photo-meta"><div class="filename" title="${esc(p.name)}">${esc(p.name)}</div><div class="photo-actions"><button class="link-edit" data-edit-photo="${p.id}">✎ Edit Foto</button><button class="link-danger" data-remove-photo="${p.id}">🗑 Hapus</button></div></div></div><div class="preset-grid">${PHOTO_PRESETS.map(pr=>`<button class="preset-btn" data-add="${p.id}|${pr.id}">+ ${pr.id}</button>`).join('')}</div><div class="custom-photo-size"><div class="custom-photo-title">Ukuran Foto Custom (mm)</div><div class="custom-photo-row"><input type="number" min="5" max="1000" step="0.1" value="40" data-custom-w><span>×</span><input type="number" min="5" max="1000" step="0.1" value="60" data-custom-h><button type="button" class="preset-btn custom-add-btn" data-add-custom="${p.id}">+ Custom</button></div></div></div>`).join('')}
...
$('#photoLibrary').addEventListener('click',e=>{const r=e.target.closest('[data-remove-photo]'),a=e.target.closest('[data-add]'),ac=e.target.closest('[data-add-custom]'),ed=e.target.closest('[data-edit-photo]');if(r)removePhoto(r.dataset.removePhoto);if(a){const [pid,pre]=a.dataset.add.split('|');addToQueue(pid,pre)}if(ac){const card=ac.closest('.photo-card'),w=card?.querySelector('[data-custom-w]')?.value,h=card?.querySelector('[data-custom-h]')?.value;addCustomToQueue(ac.dataset.addCustom,w,h)}if(ed)openEditor(ed.dataset.editPhoto)});
...
$('#paperSize').addEventListener('change',()=>{updateCustomPaperControls();invalidate()});
['customPaperW','customPaperH','margin','gap'].forEach(id=>$('#'+id).addEventListener('change',()=>invalidate()));
...
updateCustomPaperControls();
window.addEventListener('resize' ,()=>{if(state.layoutPages.length)renderWorkspace()});
restoreProject();updateBorderControls();updateGapCutControls();renderPhotos();renderQueue();renderWorkspace();updateAccessUI();initializeFirebaseAuth().catch(err=>{console.error(err);authReady=true;updateAccessUI()});loadPdfLibrary().catch(()=>{});
