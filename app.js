...
async function exportImages(type){
  if(!requireFullAccess())return;if(!state.layoutPages.length)return alert('Silakan jalankan Auto Layout terlebih dahulu!');const s=settings(),paper=PAPER_SIZES[s.paperSize],btn=$(type==='png'?'#exportPngBtn':'#exportJpgBtn'),old=btn.textContent;btn.disabled=true;btn.textContent='⟳ '+type.toUpperCase();
