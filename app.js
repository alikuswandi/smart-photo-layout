...
const PHOTO_PRESETS=[
  {id:'2x3',name:'2 x 3 cm',w:20,h:30},{id:'3x4',name:'3 x 4 cm',w:30,h:40},{id:'4x6',name:'4 x 6 cm',w:40,h:60},{id:'2R',name:'2R (6x9)',w:60,h:90},{id:'3R',name:'3R (8.9x12.7)',w:89,h:127},{id:'4R',name:'4R (10.2x15.2)',w:102,h:152},{id:'5R',name:'5R (12.7x17.8)',w:127,h:178},
  {id:'POL-MINI',name:'Polaroid Mini (54 x 86 mm)',w:54,h:86,frame:'polaroid',frameMm:{left:4,right:4,top:4,bottom:16}},
  {id:'POL-SQUARE',name:'Polaroid Square (72 x 86 mm)',w:72,h:86,frame:'polaroid',frameMm:{left:5,right:5,top:5,bottom:15}},
  {id:'POL-CLASSIC',name:'Polaroid Classic (89 x 108 mm)',w:89,h:108,frame:'polaroid',frameMm:{left:6,right:6,top:6,bottom:20}}
];
...
const STORAGE_KEY='smartPhotoLayoutProject_v6';
const LEGACY_STORAGE_KEYS=['smartPhotoLayoutProject_v5','smartPhotoLayoutProject_v4','smartPhotoLayoutProject_v3'];
...
// v17 — Firebase Authentication (password tidak disimpan di source code)
let firebaseAuth=null;
let authUser=null;
let authReady=false;

function getFirebaseSettings(){
  return window.SMART_PHOTO_FIREBASE||{};
}
function getFirebaseConfig(){
  return getFirebaseSettings().config||{};
}
function isFirebaseConfigured(){
  const c=getFirebaseConfig();
  const required=['apiKey','authDomain','projectId','appId'];
  return required.every(k=>c[k]&&!String(c[k]).includes('YOUR_'));
}
function usernameToEmail(username){
  const value=String(username||'').trim().toLowerCase();
  if(value.includes('@'))return value;
  const domain=(getFirebaseSettings().usernameDomain||'smartphotolayout.local').trim();
  return `${value}@${domain}`;
}
...
async function initializeFirebaseAuth(){
  ...
  try{
    ...
    console.warn('Firebase persistence:',err?.message||err);
  }
  firebaseAuth.onAuthStateChanged(user=>{
    ...
    console.error('Firebase auth state:',err);
    authUser=null;
    authReady=true;
    updateAccessUI();
  });
...
function showLogin(){
  setLoginMessage('');
  if(!isFirebaseConfigured())setLoginMessage('Firebase belum dikonfigurasi. Isi firebase-config.js terlebih dahulu.');
...
function updateAccessUI(){
  ...
  const login=$('#loginBtn'),print=$('#printBtn'),pdf=$('#exportPdfBtn'),png=$('#exportPngBtn'),jpg=$('#exportJpgBtn');
  if(login){
  ...
  for(const [btn,label,title] of [[png,'PNG','Unduh hasil layout PNG'],[jpg,'JPG','Unduh hasil layout JPG']]){
    if(btn){btn.textContent=unlocked?'⇩ '+label:'🔒 '+label;btn.classList.toggle('access-locked',!unlocked);btn.title=unlocked?title:'Login untuk mengunduh '+label}
  }
  if(pdf){
...
const settings=()=>({paperSize:$('#paperSize').value,customPaperWidth:Math.max(20,Number($('#customPaperWidth')?.value)||210),customPaperHeight:Math.max(20,Number($('#customPaperHeight')?.value)||297),margin:Math.max(0,Number($('#margin').value)||0),gap:Math.max(0,Number($('#gap').value)||0),smartRotation:$('#smartRotation').checked,cropMarks:$('#cropMarks').checked,gapCutLines:$('#gapCutLines').checked,gapCutStyle:$('#gapCutStyle').value||'solid',gapCutWidth:Math.max(.1,Math.min(2,Number($('#gapCutWidth').value)||.2)),gapCutColor:$('#gapCutColor').value||'#666666',photoBorder:$('#photoBorder').checked,borderColor:$('#borderColor').value||'#ffffff',borderWidth:Math.max(.1,Math.min(5,Number($('#borderWidth').value)||1)),borderStyle:$('#borderStyle').value||'solid'});
function getPaperSize(s=settings()){return s.paperSize==='CUSTOM'?{w:s.customPaperWidth,h:s.customPaperHeight,name:'Custom'}:PAPER_SIZES[s.paperSize]||PAPER_SIZES.A4}
function updateCustomPaperControls(){const custom=$('#paperSize').value==='CUSTOM';$('#customPaperControls')?.classList.toggle('hidden',!custom)}
...
function addToQueue(photoId,presetId){
  ...
  state.queue.push({
    ...
    sourcePhotoId:sourceRootId,
    presetId:preset.id,w:preset.w,h:preset.h,name:preset.name,qty:1,frame:preset.frame||null,frameMm:preset.frameMm?{...preset.frameMm}:null
  });
  renderQueue();invalidate()
}
function addCustomToQueue(photoId,kind,values){
  const source=state.photos.find(x=>x.id===photoId);if(!source)return;
  const w=Math.max(5,Number(values.w)||0),h=Math.max(5,Number(values.h)||0);if(!w||!h)return alert('Masukkan ukuran lebar dan tinggi yang valid.');
  let preset={id:'CUSTOM-'+gid(),name:`Foto Custom (${w} x ${h} mm)`,w,h};
  if(kind==='polaroid'){
    const left=Math.max(0,Number(values.left)||0),right=Math.max(0,Number(values.right)||0),top=Math.max(0,Number(values.top)||0),bottom=Math.max(0,Number(values.bottom)||0);
    if(left+right>=w||top+bottom>=h)return alert('Margin bingkai Polaroid terlalu besar untuk ukuran luar.');
    preset={id:'POL-CUSTOM-'+gid(),name:`Polaroid Custom (${w} x ${h} mm)`,w,h,frame:'polaroid',frameMm:{left,right,top,bottom}};
  }
  const queuePhotoId=gid(),sourceRootId=source.sourcePhotoId||source.id;
  state.photos.push({...source,id:queuePhotoId,sourcePhotoId:sourceRootId,isQueueClone:true});
  state.adjustments[queuePhotoId]={...DEFAULT_ADJ,...(state.adjustments[photoId]||{})};
  state.queue.push({id:gid(),photoId:queuePhotoId,sourcePhotoId:sourceRootId,presetId:preset.id,w:preset.w,h:preset.h,name:preset.name,qty:1,frame:preset.frame||null,frameMm:preset.frameMm?{...preset.frameMm}:null});
  renderQueue();invalidate();
}
function renderPhotos(){
  const el=$('#photoLibrary'),libraryPhotos=state.photos.filter(p=>!p.isQueueClone);
  if(!libraryPhotos.length){el.innerHTML='<div class="empty-mini">Belum ada foto.</div>';return}
  const normal=PHOTO_PRESETS.filter(pr=>!pr.frame),polaroid=PHOTO_PRESETS.filter(pr=>pr.frame==='polaroid');
  el.innerHTML=libraryPhotos.map(p=>`<div class="photo-card" data-photo-card="${p.id}"><div class="photo-head"><img class="thumb" src="${p.aiCutoutDataUrl||p.dataUrl}"><div class="photo-meta"><div class="filename" title="${esc(p.name)}">${esc(p.name)}</div><div class="photo-actions"><button class="link-edit" data-edit-photo="${p.id}">✎ Edit Foto</button><button class="link-danger" data-remove-photo="${p.id}">🗑 Hapus</button></div></div></div><div class="preset-label">Ukuran Foto</div><div class="preset-grid">${normal.map(pr=>`<button class="preset-btn" data-add="${p.id}|${pr.id}">+ ${pr.id}</button>`).join('')}</div><div class="custom-photo-box"><div class="custom-title">↔ Foto Custom (mm)</div><div class="custom-inline-grid"><input data-cphoto-w type="number" min="5" step="0.1" value="50" title="Lebar"><input data-cphoto-h type="number" min="5" step="0.1" value="70" title="Tinggi"></div><button class="custom-add-btn" data-add-custom-photo="${p.id}">+ Tambahkan Foto Custom</button></div><div class="preset-label polaroid-label">▣ Polaroid</div><div class="preset-grid polaroid-grid">${polaroid.map(pr=>`<button class="preset-btn polaroid-btn" data-add="${p.id}|${pr.id}" title="${pr.name}">+ ${pr.id.replace('POL-','')}</button>`).join('')}</div><div class="custom-photo-box"><div class="custom-title">▣ Polaroid Custom (mm)</div><div class="custom-inline-grid"><input data-pol-w type="number" min="10" step="0.1" value="70" title="Lebar luar"><input data-pol-h type="number" min="10" step="0.1" value="90" title="Tinggi luar"></div><div class="custom-inline-grid four" style="margin-top:6px"><input data-pol-l type="number" min="0" step="0.1" value="5" title="Bingkai kiri"><input data-pol-r type="number" min="0" step="0.1" value="5" title="Bingkai kanan"><input data-pol-t type="number" min="0" step="0.1" value="5" title="Bingkai atas"><input data-pol-b type="number" min="0" step="0.1" value="15" title="Bingkai bawah"></div><span class="custom-help">Urutan margin: kiri, kanan, atas, bawah.</span><button class="custom-add-btn" data-add-custom-polaroid="${p.id}">+ Tambahkan Polaroid Custom</button></div></div>`).join('')
}
...
function drawUprightPrintItem(ctx,source,item,wpx,hpx,a){
  const isPolaroid=item?.frame==='polaroid';
  if(!isPolaroid){
    ctx.fillStyle=a.bgEnabled?a.bgColor:'#ffffff';
    ctx.fillRect(0,0,wpx,hpx);
    drawCover(ctx,source,0,0,wpx,hpx,a.zoom,a.offsetX,a.offsetY);
    return;
  }
  const fm=item.frameMm||{left:5,right:5,top:5,bottom:16};
  const outerW=Math.max(.01,Number(item.originalW||item.w)||1),outerH=Math.max(.01,Number(item.originalH||item.h)||1);
  const sx=wpx/outerW,sy=hpx/outerH;
  const left=Math.max(0,fm.left*sx),right=Math.max(0,fm.right*sx),top=Math.max(0,fm.top*sy),bottom=Math.max(0,fm.bottom*sy);
  const iw=Math.max(1,wpx-left-right),ih=Math.max(1,hpx-top-bottom);
  ctx.fillStyle='#ffffff';ctx.fillRect(0,0,wpx,hpx);
  if(a.bgEnabled){ctx.fillStyle=a.bgColor;ctx.fillRect(left,top,iw,ih)}
  drawCover(ctx,source,left,top,iw,ih,a.zoom,a.offsetX,a.offsetY);
}
async function itemDataUrl(photoId,item,wpx,hpx){
  ...
  if(isSmartRotated){
    ...
    const upright=document.createElement('canvas');upright.width=uprightW;upright.height=uprightH;
    drawUprightPrintItem(upright.getContext('2d'),userRot,item,uprightW,uprightH,a);
    const out=document.createElement('canvas');out.width=finalW;out.height=finalH;
    const ox=out.getContext('2d');ox.fillStyle='#ffffff';ox.fillRect(0,0,finalW,finalH);
    ox.save();ox.translate(finalW/2,finalH/2);ox.rotate(Math.PI/2);ox.drawImage(upright,-uprightW/2,-uprightH/2);ox.restore();
    return out.toDataURL('image/jpeg',.95);
  }
  const c=document.createElement('canvas');c.width=finalW;c.height=finalH;
  drawUprightPrintItem(c.getContext('2d'),userRot,item,finalW,finalH,a);
...
function gapGuideSegments(page,s){
  if(!s.gapCutLines||s.gap<=0)return [];
  const items=page.items||[],eps=.08,segments=[];
  for(let i=0;i<items.length;i++)for(let j=i+1;j<items.length;j++){
    const a=items[i],b=items[j];
    let left=a,right=b;
    if(a.x>b.x){left=b;right=a}
    const hgap=right.x-(left.x+left.w), y1=Math.max(left.y,right.y),y2=Math.min(left.y+left.h,right.y+right.h);
    if(Math.abs(hgap-s.gap)<=eps&&y2-y1>.2)segments.push({dir:'v',pos:left.x+left.w+hgap/2,start:y1,end:y2});
    let top=a,bottom=b;
    if(a.y>b.y){top=b;bottom=a}
    const vgap=bottom.y-(top.y+top.h),x1=Math.max(top.x,bottom.x),x2=Math.min(top.x+top.w,bottom.x+bottom.w);
    if(Math.abs(vgap-s.gap)<=eps&&x2-x1>.2)segments.push({dir:'h',pos:top.y+top.h+vgap/2,start:x1,end:x2});
  }
  const out=[];
  for(const g of segments){if(!out.some(o=>o.dir===g.dir&&Math.abs(o.pos-g.pos)<eps&&Math.abs(o.start-g.start)<eps&&Math.abs(o.end-g.end)<eps))out.push(g)}
  return out;
}
function gapGuidePreviewHtml(page,scale,s){
  const safeMm=Math.min(s.gapCutWidth,Math.max(.05,s.gap*.8)),width=Math.max(1,safeMm*scale),color=esc(s.gapCutColor),style=s.gapCutStyle;
  return gapGuideSegments(page,s).map(g=>{
    const len=(g.end-g.start)*scale,pos=g.pos*scale,start=g.start*scale;
    if(style==='crop'){
      const mark=Math.min(len/2,Math.max(5,4*scale));
      if(g.dir==='v')return `<span class="gap-cut-line crop" style="left:${pos-width/2}px;top:${start}px;width:${width}px;height:${mark}px;background:${color}"></span><span class="gap-cut-line crop" style="left:${pos-width/2}px;top:${start+len-mark}px;width:${width}px;height:${mark}px;background:${color}"></span>`;
      return `<span class="gap-cut-line crop" style="left:${start}px;top:${pos-width/2}px;width:${mark}px;height:${width}px;background:${color}"></span><span class="gap-cut-line crop" style="left:${start+len-mark}px;top:${pos-width/2}px;width:${mark}px;height:${width}px;background:${color}"></span>`;
    }
    const dash=style==='dashed'?'repeating-linear-gradient('+(g.dir==='v'?'to bottom':'to right')+`,${color} 0,${color} ${Math.max(2,2*scale)}px,transparent ${Math.max(2,2*scale)}px,transparent ${Math.max(4,4*scale)}px)` : color;
    return g.dir==='v'?`<span class="gap-cut-line" style="left:${pos-width/2}px;top:${start}px;width:${width}px;height:${len}px;background:${dash}"></span>`:`<span class="gap-cut-line" style="left:${start}px;top:${pos-width/2}px;width:${len}px;height:${width}px;background:${dash}"></span>`;
  }).join('');
}
function drawCanvasGapGuides(ctx,page,s,pxPerMm){
  if(!s.gapCutLines)return;
  const [r,g,b]=hexToRgb(s.gapCutColor);ctx.save();ctx.strokeStyle=`rgb(${r},${g},${b})`;ctx.lineWidth=Math.max(1,Math.min(s.gapCutWidth,Math.max(.05,s.gap*.8))*pxPerMm);ctx.lineCap='butt';
  for(const x of gapGuideSegments(page,s)){
    const draw=(a,b)=>{ctx.beginPath();if(x.dir==='v'){ctx.moveTo(x.pos*pxPerMm,a*pxPerMm);ctx.lineTo(x.pos*pxPerMm,b*pxPerMm)}else{ctx.moveTo(a*pxPerMm,x.pos*pxPerMm);ctx.lineTo(b*pxPerMm,x.pos*pxPerMm)}ctx.stroke()};
    if(s.gapCutStyle==='dashed'){ctx.setLineDash([2*pxPerMm,2*pxPerMm]);draw(x.start,x.end);ctx.setLineDash([])}
    else if(s.gapCutStyle==='crop'){const m=Math.min((x.end-x.start)/2,4);draw(x.start,x.start+m);draw(x.end-m,x.end)}
    else draw(x.start,x.end);
  }
  ctx.restore();
}
function drawPdfGapGuides(doc,page,s){
  if(!s.gapCutLines)return;const [r,g,b]=hexToRgb(s.gapCutColor);doc.setDrawColor(r,g,b);doc.setLineWidth(Math.min(s.gapCutWidth,Math.max(.05,s.gap*.8)));
  for(const x of gapGuideSegments(page,s)){
    const draw=(a,b)=>x.dir==='v'?doc.line(x.pos,a,x.pos,b):doc.line(a,x.pos,b,x.pos);
    if(s.gapCutStyle==='dashed'){doc.setLineDashPattern([2,2],0);draw(x.start,x.end);doc.setLineDashPattern([],0)}
    else if(s.gapCutStyle==='crop'){const m=Math.min((x.end-x.start)/2,4);draw(x.start,x.start+m);draw(x.end-m,x.end)}
    else draw(x.start,x.end);
  }
...
async function renderWorkspace(){
  ...
  const token=++state.previewToken,s=settings(),paper=getPaperSize(s),maxW=Math.max(350,Math.min(760,$('#workspace').clientWidth-80)),scale=maxW/paper.w;
  cont.innerHTML=state.layoutPages.map((pg,idx)=>`<div><div class="page-label">Page ${idx+1} - ${paper.name||s.paperSize} (${paper.w}×${paper.h} mm) • Crop mark: ${s.cropMarks?'ON':'OFF'} • Garis gap: ${s.gapCutLines?s.gapCutStyle.toUpperCase():'OFF'} • Border: ${s.photoBorder?`${s.borderWidth} mm ${s.borderStyle}`:'OFF'}</div><div class="paper" style="width:${paper.w*scale}px;height:${paper.h*scale}px">${gapGuidePreviewHtml(pg,scale,s)}${pg.items.map(it=>`${cropMarkPreviewHtml(it,scale,s)}<div class="layout-item" data-photo-id="${it.photoId}" data-item-id="${it.uniqueId}" tabindex="0" role="button" aria-label="Edit dan atur posisi foto" style="left:${it.x*scale}px;top:${it.y*scale}px;width:${it.w*scale}px;height:${it.h*scale}px"><img id="pv-${it.uniqueId}" alt="preview" draggable="false">${borderPreviewHtml(scale,s)}</div>`).join('')}</div></div>`).join('');
...
async function buildPrintPdf(){
  ...
  const {jsPDF}=window.jspdf,s=settings(),paper=getPaperSize(s);
  ...
  */
  ...
  for(let i=0;i<state.layoutPages.length;i++){
    ...
    drawPdfGapGuides(doc,state.layoutPages[i],s);
  }
  return doc;
}
async function buildPageCanvas(page,s,paper,dpi=300){
  const pxPerMm=dpi/25.4,c=document.createElement('canvas');c.width=Math.max(1,Math.round(paper.w*pxPerMm));c.height=Math.max(1,Math.round(paper.h*pxPerMm));const x=c.getContext('2d');x.fillStyle='#ffffff';x.fillRect(0,0,c.width,c.height);
  for(const item of page.items){const dataUrl=await itemDataUrl(item.photoId,item,Math.round(item.w*pxPerMm),Math.round(item.h*pxPerMm)),img=await loadImage(dataUrl);x.drawImage(img,item.x*pxPerMm,item.y*pxPerMm,item.w*pxPerMm,item.h*pxPerMm);if(s.photoBorder){const [r,g,b]=hexToRgb(s.borderColor);x.save();x.strokeStyle=`rgb(${r},${g},${b})`;x.lineWidth=Math.max(1,s.borderWidth*pxPerMm);if(s.borderStyle==='dashed')x.setLineDash([2.5*s.borderWidth*pxPerMm,1.6*s.borderWidth*pxPerMm]);else if(s.borderStyle==='dotted')x.setLineDash([.35*s.borderWidth*pxPerMm,1.7*s.borderWidth*pxPerMm]);x.strokeRect(item.x*pxPerMm+x.lineWidth/2,item.y*pxPerMm+x.lineWidth/2,item.w*pxPerMm-x.lineWidth,item.h*pxPerMm-x.lineWidth);x.restore()}}
  drawCanvasGapGuides(x,page,s,pxPerMm);return c;
}
...
async function exportImages(type){
  if(!requireFullAccess())return;if(!state.layoutPages.length)return alert('Silakan jalankan Auto Layout terlebih dahulu!');const s=settings(),paper=getPaperSize(s),btn=$(type==='png'?'#exportPngBtn':'#exportJpgBtn'),old=btn.textContent;btn.disabled=true;btn.textContent='⟳ '+type.toUpperCase();
  try{for(let i=0;i<state.layoutPages.length;i++){const c=await buildPageCanvas(state.layoutPages[i],s,paper,300),mime=type==='png'?'image/png':'image/jpeg',url=c.toDataURL(mime,type==='png'?1:.95);downloadDataUrl(url,`Smart_Photo_Layout_Page_${i+1}.${type==='png'?'png':'jpg'}`);await new Promise(r=>setTimeout(r,150))}}
  catch(e){console.error(e);alert('Gagal membuat file '+type.toUpperCase()+'.')}finally{btn.disabled=false;btn.textContent=old}
}
...
function serializeProject(){return {version:7,savedAt:Date.now(),photos:state.photos,queue:state.queue,layoutPages:state.layoutPages,adjustments:state.adjustments,settings:settings()}}
...
function restoreProject(){try{let raw=localStorage.getItem(STORAGE_KEY);if(!raw){for(const k of LEGACY_STORAGE_KEYS){raw=localStorage.getItem(k);if(raw)break}}if(!raw)return false;const d=JSON.parse(raw);if(!d||!Array.isArray(d.photos))return false;state.photos=d.photos.map(p=>({...p,aiCutoutDataUrl:p.aiCutoutDataUrl||null}));state.queue=Array.isArray(d.queue)?d.queue:[];state.layoutPages=Array.isArray(d.layoutPages)?d.layoutPages:[];state.adjustments=d.adjustments||{};for(const p of state.photos)state.adjustments[p.id]={...DEFAULT_ADJ,...(state.adjustments[p.id]||{})};migrateQueueIndependentEdits();const s=d.settings||{};if(s.paperSize&&(s.paperSize==='CUSTOM'||PAPER_SIZES[s.paperSize]))$('#paperSize').value=s.paperSize;if(s.customPaperWidth!=null)$('#customPaperWidth').value=s.customPaperWidth;if(s.customPaperHeight!=null)$('#customPaperHeight').value=s.customPaperHeight;updateCustomPaperControls();if(s.margin!=null)$('#margin').value=s.margin;if(s.gap!=null)$('#gap').value=s.gap;if(s.smartRotation!=null)$('#smartRotation').checked=!!s.smartRotation;if(s.cropMarks!=null)$('#cropMarks').checked=!!s.cropMarks;if(s.gapCutLines!=null)$('#gapCutLines').checked=!!s.gapCutLines;if(['solid','dashed','crop'].includes(s.gapCutStyle))$('#gapCutStyle').value=s.gapCutStyle;if(s.gapCutWidth!=null)$('#gapCutWidth').value=s.gapCutWidth;if(s.gapCutColor&&/^#[0-9a-f]{6}$/i.test(s.gapCutColor)){$('#gapCutColor').value=s.gapCutColor;$('#gapCutColorText').value=s.gapCutColor;$('#gapCutColorPreset').value=s.gapCutColor}if(s.photoBorder!=null)$('#photoBorder').checked=!!s.photoBorder;if(s.borderColor&&/^#[0-9a-f]{6}$/i.test(s.borderColor)){$('#borderColor').value=s.borderColor;$('#borderColorText').value=s.borderColor}if(s.borderWidth!=null)$('#borderWidth').value=s.borderWidth;if(['solid','dashed','dotted'].includes(s.borderStyle))$('#borderStyle').value=s.borderStyle;updateBorderControls();updateGapCutControls();setStorageStatus('Project sebelumnya dipulihkan','ok');return true}catch(e){console.warn(e);return false}}
...
$('#photoLibrary').addEventListener('click',e=>{const r=e.target.closest('[data-remove-photo]'),a=e.target.closest('[data-add]'),ed=e.target.closest('[data-edit-photo]'),cp=e.target.closest('[data-add-custom-photo]'),pol=e.target.closest('[data-add-custom-polaroid]');if(r)removePhoto(r.dataset.removePhoto);if(a){const [pid,pre]=a.dataset.add.split('|');addToQueue(pid,pre)}if(ed)openEditor(ed.dataset.editPhoto);if(cp){const card=cp.closest('[data-photo-card]');addCustomToQueue(cp.dataset.addCustomPhoto,'photo',{w:card.querySelector('[data-cphoto-w]').value,h:card.querySelector('[data-cphoto-h]').value})}if(pol){const card=pol.closest('[data-photo-card]');addCustomToQueue(pol.dataset.addCustomPolaroid,'polaroid',{w:card.querySelector('[data-pol-w]').value,h:card.querySelector('[data-pol-h]').value,left:card.querySelector('[data-pol-l]').value,right:card.querySelector('[data-pol-r]').value,top:card.querySelector('[data-pol-t]').value,bottom:card.querySelector('[data-pol-b]').value})}});
...
$('#paperSize').addEventListener('change',()=>{updateCustomPaperControls();invalidate()});['customPaperWidth','customPaperHeight','margin','gap'].forEach(id=>$('#'+id).addEventListener('change',()=>invalidate()));
...
function updateGapCutControls(){const enabled=$('#gapCutLines').checked;$('#gapCutControls').classList.toggle('disabled',!enabled)}
function updateGapCutAndSave(){updateGapCutControls();renderWorkspace();scheduleSave()}
$('#gapCutLines').addEventListener('change',updateGapCutAndSave);
['gapCutStyle','gapCutWidth'].forEach(id=>$('#'+id).addEventListener('change',updateGapCutAndSave));
$('#gapCutColorPreset').addEventListener('change',e=>{$('#gapCutColor').value=e.target.value;$('#gapCutColorText').value=e.target.value;updateGapCutAndSave()});
$('#gapCutColor').addEventListener('input',e=>{$('#gapCutColorText').value=e.target.value;$('#gapCutColorPreset').value=e.target.value;updateGapCutAndSave()});
$('#gapCutColorText').addEventListener('change',e=>{let v=e.target.value.trim();if(!v.startsWith('#'))v='#'+v;if(/^#[0-9a-f]{6}$/i.test(v)){$('#gapCutColor').value=v;e.target.value=v;$('#gapCutColorPreset').value=v;updateGapCutAndSave()}else e.target.value=$('#gapCutColor').value});
...
$('#loginBtn').addEventListener('click',async()=>{
  if(hasFullAccess()){
    if(confirm('Akses penuh sedang aktif. Keluar dari akun ini?')){
...
$('#loginForm').addEventListener('submit',async e=>{
  ...
  if(!isFirebaseConfigured()||!firebaseAuth){
    setLoginMessage('Firebase belum siap. Periksa konfigurasi di firebase-config.js dan aktifkan Email/Password Authentication.');
    return;
  }
  ...
  setLoginMessage('');
  try{
    const credential=await firebaseAuth.signInWithEmailAndPassword(usernameToEmail(user),pass);
    if(!credential?.user)throw new Error('Login gagal');
    ...
    console.warn('Login gagal:',err?.message||err);
    setLoginMessage('Username atau password salah, atau akun belum aktif.');
...
restoreProject();updateCustomPaperControls();updateBorderControls();updateGapCutControls();renderPhotos();renderQueue();renderWorkspace();updateAccessUI();initializeFirebaseAuth().catch(err=>{console.error(err);authReady=true;updateAccessUI()});loadPdfLibrary().catch(()=>{});
